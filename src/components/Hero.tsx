...
const CAR1 = 'https://images.pexels.com/photos/3802510/pexels-photo-3802510.jpeg?auto=compress&cs=tinysrgb&w=800'; // Car 1 image
const CAR2 = 'https://images.pexels.com/photos/358070/pexels-photo-358070.jpeg?auto=compress&cs=tinysrgb&w=800'; // Car 2 image (different car)
const MAN = 'https://images.pexels.com/photos/4489749/pexels-photo-4489749.jpeg?auto=compress&cs=tinysrgb&w=800'; // Man image
...
const Hero: React.FC = () => {
  const [phase, setPhase] = useState(0); // 0: car1, 1: car2, 2: man, 3: done
  ...
  useEffect(() => {
    if (phase < 3) {
      ...
      // After animation, show content with a slight delay
      ...
      <div className="absolute inset-0 w-full h-full z-0">
        ...
        <img
          ...
          className={`absolute w-full h-full object-cover transition-all duration-1000
            ${phase === 0 ? 'opacity-100 top-0' : ''}
            ${phase === 1 ? 'opacity-100 top-0' : ''}
            ${phase === 2 ? 'opacity-0 top-0 -translate-x-full' : ''}
            ${phase >= 3 ? 'hidden' : ''}
          `}
          style={{transitionProperty: 'opacity, top, transform'}}
        />
        {/* Car 2 */}
        <img
          src={CAR2}
          ...
          className={`absolute w-full h-full object-cover transition-all duration-1000
            ${phase < 2 ? 'opacity-0 top-0 translate-x-full' : ''}
            ${phase === 2 ? 'opacity-100 top-0 translate-x-0' : ''}
            ${phase === 3 ? 'opacity-0 -top-32 translate-x-0' : ''}
            ${phase < 2 || phase > 3 ? 'hidden' : ''}
          `}
          style={{transitionProperty: 'opacity, top, transform'}}
        />
        {/* Man */}
        <img
          ...
          alt="Man"
          className={`absolute w-full h-full object-cover transition-all duration-1000
            ${phase < 3 ? 'opacity-0 translate-y-full' : ''}
            ${phase === 3 ? 'opacity-100 translate-y-0' : ''}
          `}
